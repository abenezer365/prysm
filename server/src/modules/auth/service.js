...
const claims = (user, sessionId) => ({
  userId: user.id,
  sessionId,
  status: user.status,
  role: user.role.code,
  clearanceRank: user.clearance.rank,
  permissions: user.role.permissions.map((x) => x.permission.code),
});
export class AuthService {
  ...
  async accessToken(user, sessionId) {
...
export const safeUser = (user, sensitive = false) => ({
  id: user.id,
  email: user.email,

...
if (!token || !investigationId) {
  throw new Error(
    "PRYSMS_TEST_ACCESS_TOKEN and PRYSMS_TEST_INVESTIGATION_ID are required",
  );
}

const socket = new WebSocket(
  process.env.PRYSMS_WS_URL || "ws://127.0.0.1:4000/api/v1/ws/chat",
);
...
socket.on("message", (raw) => {
  ...
  if (message.type === "authenticated") {
    socket.send(
      JSON.stringify({
        question: "Summarize the authorized investigation evidence.",
        investigationId,
      }),
    );
  }
  if (message.type === "done" || message.type === "error") {
    ...
    console.log(
      JSON.stringify({
        ...
        eventTypes: messages.map((item) => item.type),
      }),
    );
    socket.close();

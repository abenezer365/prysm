...
def normalize_transactions(transactions: pd.DataFrame, accounts: pd.DataFrame, invoices: pd.DataFrame) -> pd.DataFrame:
    ...
    # A dataset can legitimately have no invoice evidence. Preserve a typed
    # missing date series; pandas cannot map an empty timezone-aware series.
    issue = (tx["invoice_id"].map(invoice["issue_date"]) if len(invoice)
             else pd.Series(pd.NaT, index=tx.index, dtype="datetime64[ns, UTC]"))

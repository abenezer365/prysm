...
class Dataset:
    ...
    def __init__(self, tables, version="in-memory", checksum="in-memory"):
        ...
        self.tables = {name: tables[name].copy(deep=True) for name in self.FACTS}
        self.entities = {}
        for name, kind, key, created in (("persons", "Person", "person_id", "created_at"),
            ...
            for row in frame.to_dict("records"):
                self.entities[f"{kind}:{row[key]}"] = (utc(row[created]), row)
        ...
        for r in self.tables["relationships"].itertuples(index=False):
            self._link(f"{r.source_type}:{r.source_id}", f"{r.target_type}:{r.target_id}", r.start_time, r.end_time, "interval")
    ...
    def snapshot(self, subject, cutoff, config):
        ...
        while queue:
            key = queue.popleft()
            for neighbor, start, end, kind in sorted(self.links[key], key=lambda item: (item[0], item[1])):
        ...
        t = self.tables
        accounts = t["accounts"][("Account:"+t["accounts"].account_id).isin(seen)].copy()
        ...
        people = t["persons"][("Person:"+t["persons"].person_id).isin(seen)].copy()
        companies = t["companies"][("Company:"+t["companies"].company_id).isin(seen)].copy()

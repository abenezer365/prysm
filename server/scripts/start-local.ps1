...
Start-Process python -ArgumentList '-m','uvicorn','api.intelligence:app','--host','127.0.0.1','--port','8100' -WorkingDirectory (Join-Path $projectRoot 'ai-engine') -WindowStyle Hidden
...
& npm.cmd run build
Start-Process node -ArgumentList '--env-file=.env','src/server.js' -WorkingDirectory $serverRoot -WindowStyle Hidden

...
export async function request(
  path,
  { token, body, signal, headers, _retried = false, ...options } = {},
) {
...
export const api = {
  ...
  permissions: (token) => request("/me/permissions", { token }),
  ...
  publicChat: (body) => request("/chat/public", { method: "POST", body }),
  authorizedChat: (token, body) =>
    request("/chat/authorized", { method: "POST", token, body }),
  health: () => request("/health"),
  readiness: () => request("/health/ready"),
  dependencies: (token) => request("/health/dependencies", { token }),
  search: (token, body) => request("/search", { method: "POST", token, body }),
  ...
  updateUser: (token, id, body) =>
    request(`/users/${id}`, { method: "PATCH", token, body }),
  settings: (token, body) =>
    request("/me/settings", { method: "PATCH", token, body }),
  ...
  analyze: (token, id) =>
    request(`/investigations/${id}/analyze`, {
      method: "POST",
      token,
      body: {},
    }),
  ...
  ragIngest: (token, body) =>
    request("/rag/ingest", { method: "POST", token, body }),
  ragDocuments: (token, query = "") =>
    request(`/rag/documents${query}`, { token }),
  updateRagDocument: (token, id, enabled) =>
    request(`/rag/documents/${id}`, {
      method: "PATCH",
      token,
      body: { enabled },
    }),
  ragConversations: (token, query = "") =>
    request(`/rag/conversations${query}`, { token }),
  adminNews: (token) => request("/news/admin", { token }),
  ...
  applications: (token, query = "") =>
    request(`/applications${query}`, { token }),
  reviewApplication: (token, id, body) =>
    request(`/applications/${id}`, { method: "PATCH", token, body }),
  bugReports: (token, query = "") => request(`/bug-reports${query}`, { token }),
  updateBug: (token, id, body) =>
    request(`/bug-reports/${id}`, { method: "PATCH", token, body }),
  betaApplications: (token, query = "") =>
    request(`/beta/applications${query}`, { token }),
  reviewBeta: (token, id, body) =>
    request(`/beta/applications/${id}`, { method: "PATCH", token, body }),
  contributors: (token, query = "") =>
    request(`/contributors/applications${query}`, { token }),
  reviewContributor: (token, id, body) =>
    request(`/contributors/applications/${id}`, {
      method: "PATCH",
      token,
      body,
    }),

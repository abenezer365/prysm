...
describe("API boundary", () => {
  ...
  it("rejects public chat attempts to inject authorized context", async () => {
    const response = await request(app).post("/api/v1/chat/public").send({
      question: "secret",
  ...
  it("validates bug diagnostics and severity", async () => {
    const response = await request(app).post("/api/v1/bug-reports").send({
  ...
  it("protects settings and contributor moderation", async () => {
    ...
    const contributors = await request(app).get(
      "/api/v1/contributors/applications",
    );
